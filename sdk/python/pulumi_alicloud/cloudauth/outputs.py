# coding=utf-8
# *** WARNING: this file was generated by pulumi-language-python. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import builtins as _builtins
import warnings
import sys
import pulumi
import pulumi.runtime
from typing import Any, Mapping, Optional, Sequence, Union, overload
if sys.version_info >= (3, 11):
    from typing import NotRequired, TypedDict, TypeAlias
else:
    from typing_extensions import NotRequired, TypedDict, TypeAlias
from .. import _utilities

__all__ = [
    'GetFaceConfigsConfigResult',
]

@pulumi.output_type
class GetFaceConfigsConfigResult(dict):
    def __init__(__self__, *,
                 biz_name: _builtins.str,
                 biz_type: _builtins.str,
                 gmt_updated: _builtins.str):
        """
        :param _builtins.str biz_name: Scene name.
        :param _builtins.str biz_type: Scene type. **NOTE:** The biz_type cannot exceed 32 characters and can only use English letters, numbers and dashes (-).
        :param _builtins.str gmt_updated: The Update Time.
        """
        pulumi.set(__self__, "biz_name", biz_name)
        pulumi.set(__self__, "biz_type", biz_type)
        pulumi.set(__self__, "gmt_updated", gmt_updated)

    @_builtins.property
    @pulumi.getter(name="bizName")
    def biz_name(self) -> _builtins.str:
        """
        Scene name.
        """
        return pulumi.get(self, "biz_name")

    @_builtins.property
    @pulumi.getter(name="bizType")
    def biz_type(self) -> _builtins.str:
        """
        Scene type. **NOTE:** The biz_type cannot exceed 32 characters and can only use English letters, numbers and dashes (-).
        """
        return pulumi.get(self, "biz_type")

    @_builtins.property
    @pulumi.getter(name="gmtUpdated")
    def gmt_updated(self) -> _builtins.str:
        """
        The Update Time.
        """
        return pulumi.get(self, "gmt_updated")


