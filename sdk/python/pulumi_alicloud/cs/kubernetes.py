...
class KubernetesArgs:
    def __init__(__self__, *,
        ...
        :param pulumi.Input[str] worker_instance_charge_type: (Optional, Force new resource) Worker payment type, its valid value is either or `PostPaid` or `PrePaid`. Defaults to `PostPaid`. If value is `PrePaid`, the files `worker_period`, `worker_period_unit`, `worker_auto_renew` and `worker_auto_renew_period` are required.
