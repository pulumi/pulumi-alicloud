// *** WARNING: this file was generated by pulumi-language-java. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.alicloud.cloudmonitor.inputs;

import com.pulumi.core.Output;
import com.pulumi.core.annotations.Import;
import java.lang.Integer;
import java.lang.String;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;


public final class ServiceMetricAlarmRuleEscalationsCriticalArgs extends com.pulumi.resources.ResourceArgs {

    public static final ServiceMetricAlarmRuleEscalationsCriticalArgs Empty = new ServiceMetricAlarmRuleEscalationsCriticalArgs();

    @Import(name="comparisonOperator")
    private @Nullable Output<String> comparisonOperator;

    public Optional<Output<String>> comparisonOperator() {
        return Optional.ofNullable(this.comparisonOperator);
    }

    /**
     * Precondition for triggering a Warn-level alert.
     * 
     */
    @Import(name="preCondition")
    private @Nullable Output<String> preCondition;

    /**
     * @return Precondition for triggering a Warn-level alert.
     * 
     */
    public Optional<Output<String>> preCondition() {
        return Optional.ofNullable(this.preCondition);
    }

    @Import(name="statistics")
    private @Nullable Output<String> statistics;

    public Optional<Output<String>> statistics() {
        return Optional.ofNullable(this.statistics);
    }

    @Import(name="threshold")
    private @Nullable Output<String> threshold;

    public Optional<Output<String>> threshold() {
        return Optional.ofNullable(this.threshold);
    }

    @Import(name="times")
    private @Nullable Output<Integer> times;

    public Optional<Output<Integer>> times() {
        return Optional.ofNullable(this.times);
    }

    private ServiceMetricAlarmRuleEscalationsCriticalArgs() {}

    private ServiceMetricAlarmRuleEscalationsCriticalArgs(ServiceMetricAlarmRuleEscalationsCriticalArgs $) {
        this.comparisonOperator = $.comparisonOperator;
        this.preCondition = $.preCondition;
        this.statistics = $.statistics;
        this.threshold = $.threshold;
        this.times = $.times;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(ServiceMetricAlarmRuleEscalationsCriticalArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private ServiceMetricAlarmRuleEscalationsCriticalArgs $;

        public Builder() {
            $ = new ServiceMetricAlarmRuleEscalationsCriticalArgs();
        }

        public Builder(ServiceMetricAlarmRuleEscalationsCriticalArgs defaults) {
            $ = new ServiceMetricAlarmRuleEscalationsCriticalArgs(Objects.requireNonNull(defaults));
        }

        public Builder comparisonOperator(@Nullable Output<String> comparisonOperator) {
            $.comparisonOperator = comparisonOperator;
            return this;
        }

        public Builder comparisonOperator(String comparisonOperator) {
            return comparisonOperator(Output.of(comparisonOperator));
        }

        /**
         * @param preCondition Precondition for triggering a Warn-level alert.
         * 
         * @return builder
         * 
         */
        public Builder preCondition(@Nullable Output<String> preCondition) {
            $.preCondition = preCondition;
            return this;
        }

        /**
         * @param preCondition Precondition for triggering a Warn-level alert.
         * 
         * @return builder
         * 
         */
        public Builder preCondition(String preCondition) {
            return preCondition(Output.of(preCondition));
        }

        public Builder statistics(@Nullable Output<String> statistics) {
            $.statistics = statistics;
            return this;
        }

        public Builder statistics(String statistics) {
            return statistics(Output.of(statistics));
        }

        public Builder threshold(@Nullable Output<String> threshold) {
            $.threshold = threshold;
            return this;
        }

        public Builder threshold(String threshold) {
            return threshold(Output.of(threshold));
        }

        public Builder times(@Nullable Output<Integer> times) {
            $.times = times;
            return this;
        }

        public Builder times(Integer times) {
            return times(Output.of(times));
        }

        public ServiceMetricAlarmRuleEscalationsCriticalArgs build() {
            return $;
        }
    }

}
