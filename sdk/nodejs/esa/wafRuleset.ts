// *** WARNING: this file was generated by pulumi-language-nodejs. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as utilities from "../utilities";

/**
 * Provides a ESA Waf Ruleset resource.
 *
 * waf rule set.
 *
 * For information about ESA Waf Ruleset and how to use it, see [What is Waf Ruleset](https://next.api.alibabacloud.com/document/ESA/2024-09-10/CreateWafRuleset).
 *
 * > **NOTE:** Available since v1.260.0.
 *
 * ## Example Usage
 *
 * Basic Usage
 *
 * ```typescript
 * import * as pulumi from "@pulumi/pulumi";
 * import * as alicloud from "@pulumi/alicloud";
 *
 * const config = new pulumi.Config();
 * const name = config.get("name") || "tf-example";
 * const _default = alicloud.esa.getSites({
 *     planSubscribeType: "enterpriseplan",
 * });
 * const defaultWafRuleset = new alicloud.esa.WafRuleset("default", {
 *     siteId: _default.then(_default => _default.sites?.[0]?.siteId),
 *     phase: "http_custom",
 *     siteVersion: 0,
 *     name: name,
 * });
 * ```
 *
 * 📚 Need more examples? VIEW MORE EXAMPLES
 *
 * ## Import
 *
 * ESA Waf Ruleset can be imported using the id, e.g.
 *
 * ```sh
 * $ pulumi import alicloud:esa/wafRuleset:WafRuleset example <ruleset_id>:<site_id>
 * ```
 */
export class WafRuleset extends pulumi.CustomResource {
    /**
     * Get an existing WafRuleset resource's state with the given name, ID, and optional extra
     * properties used to qualify the lookup.
     *
     * @param name The _unique_ name of the resulting resource.
     * @param id The _unique_ provider ID of the resource to lookup.
     * @param state Any extra arguments used during the lookup.
     * @param opts Optional settings to control the behavior of the CustomResource.
     */
    public static get(name: string, id: pulumi.Input<pulumi.ID>, state?: WafRulesetState, opts?: pulumi.CustomResourceOptions): WafRuleset {
        return new WafRuleset(name, <any>state, { ...opts, id: id });
    }

    /** @internal */
    public static readonly __pulumiType = 'alicloud:esa/wafRuleset:WafRuleset';

    /**
     * Returns true if the given object is an instance of WafRuleset.  This is designed to work even
     * when multiple copies of the Pulumi SDK have been loaded into the same process.
     */
    public static isInstance(obj: any): obj is WafRuleset {
        if (obj === undefined || obj === null) {
            return false;
        }
        return obj['__pulumiType'] === WafRuleset.__pulumiType;
    }

    /**
     * The ruleset name.
     */
    declare public readonly name: pulumi.Output<string>;
    /**
     * The WAF phase
     */
    declare public readonly phase: pulumi.Output<string>;
    /**
     * waf rule set id
     */
    declare public /*out*/ readonly rulesetId: pulumi.Output<number>;
    /**
     * The website ID, which can be obtained by calling the [ListSites](https://www.alibabacloud.com/help/en/doc-detail/2850189.html) operation.
     */
    declare public readonly siteId: pulumi.Output<string>;
    declare public readonly siteVersion: pulumi.Output<number | undefined>;
    /**
     * Rule Set Status
     */
    declare public readonly status: pulumi.Output<string>;

    /**
     * Create a WafRuleset resource with the given unique name, arguments, and options.
     *
     * @param name The _unique_ name of the resource.
     * @param args The arguments to use to populate this resource's properties.
     * @param opts A bag of options that control this resource's behavior.
     */
    constructor(name: string, args: WafRulesetArgs, opts?: pulumi.CustomResourceOptions)
    constructor(name: string, argsOrState?: WafRulesetArgs | WafRulesetState, opts?: pulumi.CustomResourceOptions) {
        let resourceInputs: pulumi.Inputs = {};
        opts = opts || {};
        if (opts.id) {
            const state = argsOrState as WafRulesetState | undefined;
            resourceInputs["name"] = state?.name;
            resourceInputs["phase"] = state?.phase;
            resourceInputs["rulesetId"] = state?.rulesetId;
            resourceInputs["siteId"] = state?.siteId;
            resourceInputs["siteVersion"] = state?.siteVersion;
            resourceInputs["status"] = state?.status;
        } else {
            const args = argsOrState as WafRulesetArgs | undefined;
            if (args?.phase === undefined && !opts.urn) {
                throw new Error("Missing required property 'phase'");
            }
            if (args?.siteId === undefined && !opts.urn) {
                throw new Error("Missing required property 'siteId'");
            }
            resourceInputs["name"] = args?.name;
            resourceInputs["phase"] = args?.phase;
            resourceInputs["siteId"] = args?.siteId;
            resourceInputs["siteVersion"] = args?.siteVersion;
            resourceInputs["status"] = args?.status;
            resourceInputs["rulesetId"] = undefined /*out*/;
        }
        opts = pulumi.mergeOptions(utilities.resourceOptsDefaults(), opts);
        super(WafRuleset.__pulumiType, name, resourceInputs, opts);
    }
}

/**
 * Input properties used for looking up and filtering WafRuleset resources.
 */
export interface WafRulesetState {
    /**
     * The ruleset name.
     */
    name?: pulumi.Input<string>;
    /**
     * The WAF phase
     */
    phase?: pulumi.Input<string>;
    /**
     * waf rule set id
     */
    rulesetId?: pulumi.Input<number>;
    /**
     * The website ID, which can be obtained by calling the [ListSites](https://www.alibabacloud.com/help/en/doc-detail/2850189.html) operation.
     */
    siteId?: pulumi.Input<string>;
    siteVersion?: pulumi.Input<number>;
    /**
     * Rule Set Status
     */
    status?: pulumi.Input<string>;
}

/**
 * The set of arguments for constructing a WafRuleset resource.
 */
export interface WafRulesetArgs {
    /**
     * The ruleset name.
     */
    name?: pulumi.Input<string>;
    /**
     * The WAF phase
     */
    phase: pulumi.Input<string>;
    /**
     * The website ID, which can be obtained by calling the [ListSites](https://www.alibabacloud.com/help/en/doc-detail/2850189.html) operation.
     */
    siteId: pulumi.Input<string>;
    siteVersion?: pulumi.Input<number>;
    /**
     * Rule Set Status
     */
    status?: pulumi.Input<string>;
}
