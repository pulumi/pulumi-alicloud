 ...
 *
 * const config = new pulumi.Config();
 * const name = config.get("name") || "tf-example-resource-pool-options";
 * const _default = alicloud.getZones({
 *     availableResourceCreation: "VSwitch",
 * });
 * const cloudEssd = _default.then(_default => alicloud.ecs.getInstanceTypes({
 ...
 * });
 * const vsw1 = Promise.all([defaultGetNetworks, defaultGetNetworks.then(defaultGetNetworks => std.cidrsubnet({
 *     input: defaultGetNetworks.vpcs?.[0]?.cidrBlock,
 *     newbits: 8,
 *     netnum: 8,
 * }))]).then(([defaultGetNetworks, invoke]) => alicloud.vpc.getSwitches({
 *     vpcId: defaultGetNetworks.ids?.[0],
 *     cidrBlock: invoke.result,
 * }));
 * const defaultGetManagedKubernetesClusters = alicloud.cs.getManagedKubernetesClusters({
 *     nameRegex: "^cluster-",
 * });
 ...
 *     instanceAmount: 1,
 *     instanceType: cloudEssd.then(cloudEssd => cloudEssd.instanceTypes?.[0]?.id),
 *     matchCriteria: "Target",
 *     zoneIds: [_default.then(_default => _default.zones?.[0]?.id)],
 * });
 ...
 * const privatePoolFirst = new alicloud.cs.NodePool("private_pool_first", {
 *     nodePoolName: `${name}-first`,
 *     clusterId: defaultGetManagedKubernetesClusters.then(defaultGetManagedKubernetesClusters => defaultGetManagedKubernetesClusters.ids?.[0]),
 *     vswitchIds: [vsw1.then(vsw1 => vsw1.ids?.[0])],
 *     instanceChargeType: "PostPaid",
 *     systemDiskCategory: "cloud_essd",
 *     systemDiskSize: 120,
 *     multiAzPolicy: "PRIORITY",
 *     desiredSize: "0",
 *     instanceTypes: [cloudEssd.then(cloudEssd => cloudEssd.instanceTypes?.[0]?.id)],
 *     resourcePoolOptions: {
 ...
 * const privatePoolOnly = new alicloud.cs.NodePool("private_pool_only", {
 *     nodePoolName: `${name}-only`,
 *     clusterId: defaultGetManagedKubernetesClusters.then(defaultGetManagedKubernetesClusters => defaultGetManagedKubernetesClusters.ids?.[0]),
 *     vswitchIds: [vsw1.then(vsw1 => vsw1.ids?.[0])],
 *     instanceChargeType: "PostPaid",
 *     systemDiskCategory: "cloud_essd",
...
export class NodePool extends pulumi.CustomResource {
     ...
     */
    declare public readonly resourcePoolOptions: pulumi.Output<outputs.cs.NodePoolResourcePoolOptions | undefined>;
    /**
    ...
    constructor(name: string, argsOrState?: NodePoolArgs | NodePoolState, opts?: pulumi.CustomResourceOptions) {
        ...
        if (opts.id) {
            ...
            resourceInputs["resourcePoolOptions"] = state?.resourcePoolOptions;
            ...
            resourceInputs["resourcePoolOptions"] = args?.resourcePoolOptions;
...
export interface NodePoolState {
     ...
     * > **NOTE:** `resourcePoolOptions` only takes effect when creating PostPaid (pay-as-you-go) instances, and it cannot be set together with `private_pool_options.private_pool_options_match_criteria` and `private_pool_options.private_pool_options_id`.
     */
    resourcePoolOptions?: pulumi.Input<inputs.cs.NodePoolResourcePoolOptions | undefined>;
    /**
     * Rotary configuration. See `rollingPolicy` below.
     *
     * > **NOTE:** This parameter only applies during resource update. If modified in isolation without other property changes, Terraform will not trigger any action.
