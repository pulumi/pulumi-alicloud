...
function parseFunctionCode(funcString) {
    ...
    if (funcString.startsWith("class ")) {
        ...
        if (diagnostics.length) {
            return [`the class could not be parsed: ${diagnostics[0].messageText}`, undefined];
        }
    ...
    function makeFunctionDeclaration(v, isFunctionDeclaration) {
...
function createSourceFile(serializedFunction) {
    ...
    const file = ts.createSourceFile("", toParse, ts.ScriptTarget.Latest, true, ts.ScriptKind.TS);
    const diagnostics = file.parseDiagnostics;
    if (diagnostics.length) {
