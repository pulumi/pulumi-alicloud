...
//					},
//				},
//				ContainerdConfig: &cs.NodePoolContainerdConfigArgs{
//					MaxConcurrentDownloads:   pulumi.Int(10),
//					IgnoreImageDefinedVolume: pulumi.String("true"),
//					LimitCore:                pulumi.String("10"),
//					LimitNoFile:              pulumi.String("1024"),
//					LimitMemLock:             pulumi.String("65536"),
//					RegistryMirrors: pulumi.StringArray{
//						pulumi.String("docker.io=https://registry.cn-hangzhou.aliyuncs.com,https://mirror2.example.com&override_path"),
//						pulumi.String("gcr.io=https://gcr-mirror.example.com&override_path"),
//					},
//					InsecureRegistries: pulumi.StringArray{
//						pulumi.String("registry.example.com"),
//						pulumi.String("192.168.1.1:5000"),
//					},
//				},
...
// ```
//
// NodePool with resourcePoolOptions (private pool):
//
// Create Target mode private pools with `ecs.CapacityReservation` and `ecs.ElasticityAssurance`, then reference them in the node pool via `resourcePoolOptions`. To use a private pool, the node pool must specify `instanceTypes` that include the private pool's instance type, and its vSwitch must be in the same zone as the private pool.
//
// ```go
// package main
//
...
//				return err
//			}
//			cloudEssd, err := ecs.GetInstanceTypes(ctx, &ecs.GetInstanceTypesArgs{
...
//				NameRegex: pulumi.StringRef("^default-NODELETING-ACK$"),
//			}, nil)
//			if err != nil {
//				return err
//			}
...
//				return err
//			}
//			vsw1, err := vpc.GetSwitches(ctx, &vpc.GetSwitchesArgs{
//				VpcId:     pulumi.StringRef(defaultGetNetworks.Ids[0]),
//				CidrBlock: pulumi.StringRef(invokeCidrsubnet.Result),
//			}, nil)
//			if err != nil {
//				return err
//			}
//			defaultGetManagedKubernetesClusters, err := cs.GetManagedKubernetesClusters(ctx, &cs.GetManagedKubernetesClustersArgs{
//				NameRegex: pulumi.StringRef("^cluster-"),
//			}, nil)
//			if err != nil {
//				return err
//			}
//			// Target mode private pool generated by a capacity reservation service.
//			defaultCapacityReservation, err := ecs.NewCapacityReservation(ctx, "default", &ecs.CapacityReservationArgs{
//				Description:             pulumi.String(name),
//				Platform:                pulumi.String("linux"),
//				CapacityReservationName: pulumi.String(name),
//				EndTimeType:             pulumi.String("Unlimited"),
//				InstanceAmount:          pulumi.Int(1),
//				InstanceType:            pulumi.String(cloudEssd.InstanceTypes[0].Id),
//				MatchCriteria:           pulumi.String("Target"),
...
//			defaultElasticityAssurance, err := ecs.NewElasticityAssurance(ctx, "default", &ecs.ElasticityAssuranceArgs{
//				InstanceAmount: pulumi.Int(1),
//				Description:    pulumi.String(name),
//				ZoneIds: pulumi.StringArray{
//					pulumi.String(_default.Zones[0].Id),
...
//				Period:                          pulumi.Int(1),
//				PrivatePoolOptionsMatchCriteria: pulumi.String("Target"),
//				InstanceType:                    pulumi.String(cloudEssd.InstanceTypes[0].Id),
//				PeriodUnit:                      pulumi.String("Month"),
...
//			_, err = cs.NewNodePool(ctx, "private_pool_first", &cs.NodePoolArgs{
//				NodePoolName: pulumi.Sprintf("%v-first", name),
//				ClusterId:    pulumi.String(defaultGetManagedKubernetesClusters.Ids[0]),
//				VswitchIds: pulumi.StringArray{
//					pulumi.String(vsw1.Ids[0]),
//				},
//				InstanceChargeType: pulumi.String("PostPaid"),
//				SystemDiskCategory: pulumi.String("cloud_essd"),
//				SystemDiskSize:     pulumi.Int(120),
//				MultiAzPolicy:      pulumi.String("PRIORITY"),
//				DesiredSize:        pulumi.String("0"),
//				InstanceTypes: pulumi.StringArray{
//					pulumi.String(cloudEssd.InstanceTypes[0].Id),
//				},
//				ResourcePoolOptions: &cs.NodePoolResourcePoolOptionsArgs{
//					Strategy: pulumi.String("PrivatePoolFirst"),
//					PrivatePoolIds: pulumi.StringArray{
//						defaultCapacityReservation.ID().ToIDOutput().ToStringOutput(),
//					},
//				},
//			})
//			if err != nil {
...
//			_, err = cs.NewNodePool(ctx, "private_pool_only", &cs.NodePoolArgs{
//				NodePoolName: pulumi.Sprintf("%v-only", name),
//				ClusterId:    pulumi.String(defaultGetManagedKubernetesClusters.Ids[0]),
//				VswitchIds: pulumi.StringArray{
//					pulumi.String(vsw1.Ids[0]),
//				},
...
//				},
//				ResourcePoolOptions: &cs.NodePoolResourcePoolOptionsArgs{
//					Strategy: pulumi.String("PrivatePoolOnly"),
//					PrivatePoolIds: pulumi.StringArray{
//						defaultCapacityReservation.ID().ToIDOutput().ToStringOutput(),
//						defaultElasticityAssurance.ElasticityAssuranceId,
//					},
//				},
//			})
//			if err != nil {
...
// ## Import
//
// Container Service for Kubernetes (ACK) Nodepool can be imported using the id, which consists of clusterId and node_pool_id, e.g.
//
// ```sh
...
type NodePool struct {
	...
	ContainerdConfig NodePoolContainerdConfigPtrOutput `pulumi:"containerdConfig"`
	...
	ResourcePoolOptions NodePoolResourcePoolOptionsPtrOutput `pulumi:"resourcePoolOptions"`
...
type nodePoolState struct {
	...
	//
	// > **NOTE:** Setting `containerdConfig` at creation time takes effect through an extra nodeConfig update call issued after the node pool has been created. Removing the whole `containerdConfig` block clears all custom containerd configuration on the cloud side (the API uses full-replacement semantics); an empty block is equivalent to omitting the parameter. See `containerdConfig` below.
	ContainerdConfig *NodePoolContainerdConfig `pulumi:"containerdConfig"`
	// Node CPU management policies. Default value: `none`. When the cluster version is 1.12.6 or later, the following two policies are supported:
	...
	//
	// > **NOTE:** `resourcePoolOptions` only takes effect when creating PostPaid (pay-as-you-go) instances, and it cannot be set together with `private_pool_options.private_pool_options_match_criteria` and `private_pool_options.private_pool_options_id`.
	ResourcePoolOptions *NodePoolResourcePoolOptions `pulumi:"resourcePoolOptions"`
	// Rotary configuration. See `rollingPolicy` below.
	//
	// > **NOTE:** This parameter only applies during resource update. If modified in isolation without other property changes, Terraform will not trigger any action.
...
type NodePoolState struct {
	...
	//
	// > **NOTE:** Setting `containerdConfig` at creation time takes effect through an extra nodeConfig update call issued after the node pool has been created. Removing the whole `containerdConfig` block clears all custom containerd configuration on the cloud side (the API uses full-replacement semantics); an empty block is equivalent to omitting the parameter. See `containerdConfig` below.
	ContainerdConfig NodePoolContainerdConfigPtrInput
	// Node CPU management policies. Default value: `none`. When the cluster version is 1.12.6 or later, the following two policies are supported:
	...
	//
	// > **NOTE:** `resourcePoolOptions` only takes effect when creating PostPaid (pay-as-you-go) instances, and it cannot be set together with `private_pool_options.private_pool_options_match_criteria` and `private_pool_options.private_pool_options_id`.
	ResourcePoolOptions NodePoolResourcePoolOptionsPtrInput
	// Rotary configuration. See `rollingPolicy` below.
	//
	// > **NOTE:** This parameter only applies during resource update. If modified in isolation without other property changes, Terraform will not trigger any action.
...
//
// > **NOTE:** Setting `containerdConfig` at creation time takes effect through an extra nodeConfig update call issued after the node pool has been created. Removing the whole `containerdConfig` block clears all custom containerd configuration on the cloud side (the API uses full-replacement semantics); an empty block is equivalent to omitting the parameter. See `containerdConfig` below.
func (o NodePoolOutput) ContainerdConfig() NodePoolContainerdConfigPtrOutput {
	return o.ApplyT(func(v *NodePool) NodePoolContainerdConfigPtrOutput { return v.ContainerdConfig }).(NodePoolContainerdConfigPtrOutput)
}
...
// An KMS encryption context used to decrypt `kmsEncryptedPassword` before creating or updating a cs kubernetes with `kmsEncryptedPassword`. See [Encryption Context](https://www.alibabacloud.com/help/doc-detail/42975.htm). It is valid when `kmsEncryptedPassword` is set.
func (o NodePoolOutput) KmsEncryptionContext() pulumi.StringMapOutput {
	return o.ApplyT(func(v *NodePool) pulumi.StringMapOutput { return v.KmsEncryptionContext }).(pulumi.StringMapOutput)
}
...
//
// > **NOTE:**  This parameter is only supported for ACK-managed clusters of 1.22 or later versions.
func (o NodePoolOutput) RamRoleName() pulumi.StringOutput {
	return o.ApplyT(func(v *NodePool) pulumi.StringOutput { return v.RamRoleName }).(pulumi.StringOutput)
}
...
//
// > **NOTE:** `resourcePoolOptions` only takes effect when creating PostPaid (pay-as-you-go) instances, and it cannot be set together with `private_pool_options.private_pool_options_match_criteria` and `private_pool_options.private_pool_options_id`.
func (o NodePoolOutput) ResourcePoolOptions() NodePoolResourcePoolOptionsPtrOutput {
	return o.ApplyT(func(v *NodePool) NodePoolResourcePoolOptionsPtrOutput { return v.ResourcePoolOptions }).(NodePoolResourcePoolOptionsPtrOutput)
}
